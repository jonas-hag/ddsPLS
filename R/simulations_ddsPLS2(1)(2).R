...
do_sPLS_one_comp <- function(x,y,Ks=NULL,kYs=NULL,NCORES=1,tau=0.0975){
  ...
  PRESS_il_y_and_i_l <- foreach(pos_decoupe=1:min(NCORES,n_paras),.packages = c("mixOmics")) %my_do% {
    ...
    for(i in 1:n_i){
      ...
      SE[i,] <- (Y_est-Y_test)^2
      var_sel[i,which(abs(c(model$loadings$Y))>1e-9)] <- 1
    }
  ...
  for(i in 1:n_paras){
    ...
    for(j in 1:length(pos)){
      ...
      var_sel_j <- which(PRESS_il_y_and_i_l[[pos_list[j]]]$var_sel[pos_in_list[j],]!=0)
...
do_sPLS_all_comp <- function(x,y,Ks=NULL,kYs=NULL,B_th,tau=0.0975,NCORES=1,NZV = 1e-4){
  n <- nrow(y);p <- ncol(x);q <- ncol(y)
  if(is.null(Ks)){
    Ks <- 1:p
  }
  ...
  while(test){
    ...
    if(is.null(ONE_COMP)){
      ...
      if(h==0){
        ...
        Q2_cum <- 1-sum(ONE_COMP$PRESS[id_all,])/sum(ONE_COMP$RSS)
        Q2_cum_star <- ONE_COMP$Q2_h_sum_star[id_all]
      }else{
        id_kX <- which(ONE_COMP$paras_K_X_Y[,1]==ONE_COMP$kX_best)
        id_kY <- which(ONE_COMP$paras_K_X_Y[,2]==ONE_COMP$kY_best)
        id_all <- intersect(id_kX,id_kY)
        Q2_cum_pot <- 1-(1-Q2_cum)*sum(ONE_COMP$PRESS[id_all,])/sum(ONE_COMP$RSS)
        Q2_cum_star_pot <- 1-(1-Q2_cum_star)*ONE_COMP$Q2_h_sum_star[id_all]
        if(Q2_cum_star_pot>Q2_cum_star){
          Q2_cum_star <- Q2_cum_star_pot
      ...
      if(test){
        ...
        RSS[[h+1]] <- ONE_COMP$RSS
        Q2_reg <- 1 - sum(PRESS[[h+1]][id_all,])/sum(RSS[[1]])
        # STAR
        ...
        Var_h <- sum(scale(y_res,scale = F)^2)/sum(RSS[[1]])
        if(Var_h<NZV){
          test <- F
        }
        h <- h + 1
  ...
  if(h!=0){
    ...
    model <- mixOmics::spls(x,y,ncomp = R_hat,keepX = best_keepXs,keepY = best_keepYs)
    y_sc <- scale(y,scale = F)
    Q2_reg_star <- 1-PRESS_star_sel/sum(y_sc[,as.vector(which(abs(rowSums(model$loadings$Y))>1e-9))]^2)
    B_hat <- predict(model,newdata = x)$B.hat[,,R_hat]
    stat_sel <- compare_selection(B_hat,B_th)
    dist_B <- sum((B_hat-B_th)^2)
    out <- list(R_hat=R_hat,B_hat=B_hat,SEL=stat_sel,
                Q2_cum=Q2_cum,Q2_reg=Q2_reg,
                Q2_cum_star=Q2_cum_star,Q2_reg_star=Q2_reg_star,
                best_keepXs=best_keepXs,best_keepYs=best_keepYs)
  }
}
...
test <- function(){
  if(T){
    ...
    lambdas <- seq(0.05,0.99,length.out = 50)
  ...
  for(i in 1:NNs){
    ...
    # datas$Y[[i]] <- Y
    if(i%%5==0){
      save(datas,df,LAMBDAS_SOL,file = "../../Hadrien/data_signalFort_no_1_2.RData")#save(datas,df,file = "../data_simu/data_signalFaible.RData")
    }
    # Load data
    datas$Xs[[i]] -> Xs
    datas$Y[[i]] -> Y
    ...
    for(i_m in 1:length(pos)){
      ...
      toPlot <- T#method_i %in% method[c(1,4)]
      if(toPlot){#T){#
        if(method_i %in% c("ddsPLS_local","ddsPLS_global","PLS")){
          ...
          res <- Q2_ddsPLS(Xs,Y,lambdas = lambdas_i,tau=0.0975,NZV=NZV,
                           type = "local",NCORES=3)
          if(!is.null(res)){
          ...
          res <- tryCatch({
            do_sPLS_all_comp(as.data.frame(x),Y,B_th=B_th,Ks=1:8,kYs=1:3,NCORES = 10)
          ...
          res = tryCatch({
            Q2_chun(x,Y,K=5,B_th=B_th,etas=etas,kappas=kappas,NCORES = length(kappas))
          }, error = function(error_condition) {
            NULL
          })
          if(!is.null(res)){
          ...
          df[pos_i,]$SE_B <- sum((MASS::ginv(do.call(cbind,Xs))%*%Y-B_th_all)^2)
          df[pos_i,id_sel] <- sel_no_sp
    ...
    ##########################
    if(T){
      pdf(file = paste("/Users/hlorenzo/Dropbox/vizu_Chun_et_all.pdf",sep=""),width = 24,height = 15)
      ...
      cols <- RColorBrewer::brewer.pal(length(method),"Paired")

      boxplot(Q2_CUM_star~method*n,col=cols[rep(1:l_m,i)],df,main=expression("Q"[cum]^2~"*"),las=2,xlab="")
      abline(h=0.0975)
      abline(v=0.5+c(0:(length(ns)) )*length(levels(df$method)),lty=3)
      legend("bottomright",legend = levels(df$method),fill = cols,bty="n")
      boxplot(Q2_CUM~method*n,col=cols[rep(1:l_m,i)],df,main=expression("Q"[cum]^2),las=2,xlab="")
      abline(h=0.0975)
      abline(v=0.5+c(0:(length(ns)) )*length(levels(df$method)),lty=3)
      legend("bottomright",legend = levels(df$method),fill = cols,bty="n")

      boxplot(Q2_star~method*n,col=cols[rep(1:l_m,i)],df,main=expression("Q"^2~"*"),las=2,xlab="")
      abline(h=0.0975)
      abline(v=0.5+c(0:(length(ns)) )*length(levels(df$method)),lty=3)
      legend("bottomright",legend = levels(df$method),fill = cols,bty="n")
      boxplot(Q2~method*n,col=cols[rep(1:l_m,i)],df,main=expression("Q"^2),las=2,xlab="",ylim=c(-0.5,1))
      abline(h=0.0975)
      abline(v=0.5+c(0:(length(ns)) )*length(levels(df$method)),lty=3)
      legend("bottomright",legend = levels(df$method),fill = cols,bty="n")

      boxplot(SE_B~method*n,df[-which(df$method=="OLS_esp"),],main=expression('||'~hat(B)-A^"+"*C~'||/||'~A^"+"*C~'||'),log="y",
              col=cols[rep(1:l_m,i)],las=2,xlab="");abline(h=0,lty=2)
      abline(v=0.5+c(0:(length(ns)) )*length(unique(df$method)),lty=3)
      legend("topright",legend = levels(df$method),fill = cols,bty="n")
      ...
      boxplot(R~method*n,df,main="R",
              col=cols[rep(1:l_m,i)],las=2,xlab="")
