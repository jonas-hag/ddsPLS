...
```{r, fig.show='hold',fig.width=7, fig.height=5,message=FALSE}
data("penicilliumYES")
X <- penicilliumYES$X
...
res_cv_class <- perf_mddsPLS(X,Y,R = 2,lambda_min=0.9,n_lambda=n_lambda,
                             mode = "clas",NCORES = NCORES,
                             fold_fixed = rep(1:12,3))
plot(res_cv_class,legend_names = levels(Y),pos_legend="topleft")
```

# Multi-block simulation case
A simulation process is proposed to getan idea of the behavior of the method in the multi-block context with missing values.
...
## Start cross-validation leave-one-out validation
