...
<i class="fa fa-exclamation-triangle fa-3x" aria-hidden="true"></i> **No individual can be missing in all the $\mathbf{X}$ matrices and no missing entry is allowed in the $\mathbf{Y}$ dataset** <i class="fa fa-exclamation-triangle fa-1x" aria-hidden="true"></i>
...
Cross-validation is at the core of the method and the following figure explains, for a $T$-blocks data-set how can be spred the missing values and how is split in $train$ and $test$ parts.

![Structure of a $T$-blocks data set for $train$ (resp. $test$) part, denoted by **o**
(resp. **b**) symbol. Missing values are symbolized by hatched areas.](missing_structure.png)
...
The following figure permits to represent the *weights* for the first and the third components for block 1 (mono-block case actually) and block Y. **mar_left** parameter adds extra values on the left of the plot, usefull to see variable names.

```{r}
plot(mddsPLS_model_reg,vizu = "weights",comp = c(1,3),addY = T,mar_left = 2)
```

##### *Heatmap* representation

Heatmap permit to appreciate the discriminative aspect of variables according to their individual representations. It has been decided to plot heatmaps component per component, first and second her.
...
```{r, fig.show='hold',message=FALSE,eval=T,fig.width=12, fig.height=7}
plot(res_cv_reg_L0,which_sd_plot = c(5,7),ylim=c(-0.3,1.1),alpha.f = 0.4,
...
The following figure permits to represent the *weights* for the first and the second components for block 1 (mono-block case actually) and block Y. **mar_left** parameter adds extra values on the left of the plot, usefull to see variable names.

```{r}
plot(mddsPLS_model_class,vizu = "weights",comp = c(1,2),addY = T,mar_left = 2)
```
...
load(file="res_cv_class_L0.RData")
```
...
plot(model_lambda,super = T,addY = T)
```
...
The following figure permits to represent the *weights* for the second and the third components for blocks 1, 2 and block Y. **mar_left** parameter adds extra values on the left of the plot, usefull to see variable names.

```{r,fig.height=14,fig.width=11}
plot(model_multi_vizu,vizu = "weights",comp = c(2,3),
     block=c(1,2) ,addY = T,mar_left = 5)
```

#### *Super-Weight* representation

The following figure permits to represent the *super-weights* for blocks 1, 2 and block Y. **mar_left** parameter adds extra values on the left of the plot, usefull to see variable names.

```{r,fig.height=14,fig.width=8}
plot(model_multi_vizu,vizu = "weights",super = T,
     block=c(1,2) ,addY = T,mar_left = 5)
```
...
```{r}
model <- mddsPLS(Xs,Y,lambda = 0.55,R = 3,verbose = T)
```
...
The following figure permits to represent the *weights* for the second and the third components for blocks 1, 2, 3, 5 and block Y. **mar_left** parameter adds extra values on the left of the plot, usefull to see variable names.

```{r,fig.height=14,fig.width=11}
plot(model,vizu = "weights",comp = c(2,3),
     block=c(1,2,4,5) ,addY = T,mar_left = 0)
```
