...
def getResult(dic):
	...
	for i in range(l_p_h):
		...
		if mode=="reg":
			...
			Y_train = Y[pos_train]
			Y_test = Y[pos_test]
		mod_0 = mddsPLS(Xs=X_train,Y=Y_train,lambd=lambd,R=R,mode=mode,
...
def reshape_dict(Xs):
	test_matrix = type(Xs)==np.ndarray
	if test_matrix:
		Xs_w = {}
		Xs_w[0] = Xs
	else:
	...
	if max(how_much_dim) == 1:
		...
		for k in range(K):
			if how_much_dim[k]==1:
				Xs_here = np.zeros((Xs_w[k].shape[0],1))
				Xs_here[:,0] = Xs_w[k]
				Xs_w[k] = Xs_here
	return Xs_w;
...
class mddsPLS:
			...
			if K>1:
				...
				if sum(sum(abs(mod_0["t"])))!=0:
					...
					while (iterat < maxIter_imput)&(err>errMin_imput):
						...
						for k in range(K):
							if len(id_na[k])>0:
								...
								if(len(Var_selected_k)>0):
									...
									model_here = Model(u=model_here_0["u"],v=model_here_0["v"],ts=model_here_0["ts"],
								  ...
								  lambd=model_here_0["lambd"])
									mod_i_k = mddsPLS(Xs=Xs_i,Y=Y_i_k,lambd=lambd,R=R,
						...
						mod_0 = mod
			self.model = Model(u=mod["u"],v=mod["v"],ts=mod["ts"],
	...
	def fill_X_test(self,X_test_0):
		...
		for k in range(K):
			id_na_test[k] = (np.isnan(X_test[k][:,0]))[0]*1
		...
		if na_test_lengths != 0:
			...
			## General model
			model_impute_test = mddsPLS(Xs=t_X_here,Y=vars_Y_here,
				lambd=lambd,R=R,
				maxIter_imput=self.maxIter_imput)
			## Create test dataset
			n_test = 1
	...
	def predict(self,newX):
		...
		q = mod.q
		if n_new==1:
			id_na_test,na_test_lengths = range(K),0
			# id_na_test : blocks with missing value
			na_test_lengths = 0
			for k in range(K):
				id_na_test[k] = np.isnan(newX_w[k][0,0])*1
				na_test_lengths = na_test_lengths + id_na_test[k]
			if na_test_lengths!=0:
...
def perf_mddsPLS(Xs,Y,lambd_min=0,lambd_max=None,n_lambd=1,lambds=None,R=1,kfolds="loo",mode="reg",fold_fixed=None,maxIter_imput=5,errMin_imput=1e-9,NCORES=1):
	...
	if lambds==None:
		if lambd_max == None:
			MMss0 = mddsPLS(Xs,Y,lambd = 0,R = 1,mode = mode,maxIter_imput = 0).model.Ms
			K = len(MMss0)
			lambd_max_w = 0
			for k in range(K):
...
if __name__ == "__main__":
	...
	X1[:,1] = 1
	X2 = df[:,[3,7]]
	# Xs is a dictionnary of array elements
	Xs = {0:X0,1:X1,2:X2}
	...
	lambd=0.6
	n_test = range(100)
	newX = {0:X0[n_test,:],1:X1[n_test,:],2:X2[n_test,:]}
	
	## Regression mode
	mod_0=mddsPLS(Xs,Y,lambd=lambd,R=R,mode="reg",verbose=True)
	Y_est_reg = mod_0.predict(newX)		
	print perf_mddsPLS(Xs,Y,R=1,kfolds=10,n_lambd=10,NCORES=6,mode="reg")
	
	## Classification mode
	mod_0_classif=mddsPLS(Xs,Y_bin,lambd=lambd,R=R,mode="clas",verbose=True)
	Y_est = mod_0_classif.predict(newX)
	#print(classification_report(Y_bin[n_test], Y_est))
	print perf_mddsPLS(Xs,Y_classif,R=1,kfolds=10,n_lambd=10,NCORES=6,mode="classif")
	
	
	
#	import matplotlib.pyplot as plt
#	plt.plot(Y_est_reg[np.where(Y_est==1.)[0]], Y[np.where(Y_est==1.)[0]], "o",c='C0')
#	plt.plot(Y_est_reg[np.where(Y_est==0.)[0]], Y[np.where(Y_est==0.)[0]], "o",c='C1')
